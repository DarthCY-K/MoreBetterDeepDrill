...
{
    public class MBDD_CompCreatesInfestations : ThingComp
    {
        ...
        public bool CanCreateInfestationNow
        {
            get
            {
                MBDD_CompDeepDrill comp = parent.GetComp<Comp.MBDD_CompDeepDrill>();
        ...
        public bool CantFireBecauseSomethingElseCreatedInfestationRecently
        {
            get
            {
                ...
                {
                    if (list[i] != parent && list[i].Position.InHorDistOf(parent.Position, 10f) && list[i].TryGetComp<Comp.MBDD_CompCreatesInfestations>().CantFireBecauseCreatedInfestationRecently)
                    {
                        return true;
                    }
                }
